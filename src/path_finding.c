...
/*   Updated: 2021/02/28 09:31:43 by eprusako         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_path		*save_path(int *visited, int find_path)
{
	t_path	*head;
	t_path	*tmp;
	int		prev;
...
t_path **check_path(int *visited, int find_path, t_path **path, int *path_no)
{
	t_path	*found_path;

	found_path = save_path(visited, find_path);
	if (!found_path)
		return (path);
